...
class CreateClientIntegrationTest
 ...
 protected:
  CreateClientIntegrationTest() = default;
...
void InsertReadDelete(Client client, std::string const& bucket_name,
...
TEST_F(CreateClientIntegrationTest, Default) {
  if (UsingEmulator()) GTEST_SKIP();
  auto client = Client::CreateDefaultClient();
  ASSERT_THAT(client, IsOk());

  ASSERT_NO_FATAL_FAILURE(InsertReadDelete(
      *client, bucket_name(), MakeRandomObjectName(), LoremIpsum()));
}

TEST_F(CreateClientIntegrationTest, WithPolicies) {
  if (UsingEmulator()) GTEST_SKIP();
